...
class RequirementForm(forms.Form):
    ...
    req_form_name = forms.CharField()
    req_form_beschreibung = forms.CharField()
    req_form_kommentar = forms.CharField()
...
class TestCaseForm(forms.Form):
    ...
    testc_form_fk_requirement = forms.ModelMultipleChoiceField(queryset=requirement.objects.all(), widget=forms.CheckboxSelectMultiple)
...
class TestRunForm(forms.Form):
    # Felder die verändert werden können
    testr_form_name = forms.CharField()
    testr_form_beschreibung = forms.CharField()
    testr_form_kommentar = forms.CharField()

    testr_form_status = forms.ChoiceField(choices=RUN_STATUS)

    testr_form_fk_testcase = forms.ModelChoiceField(queryset=testcase.objects.all())
